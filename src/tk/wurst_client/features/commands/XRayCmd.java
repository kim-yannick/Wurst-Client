...
public class XRayCmd extends Cmd
{
	...
	public void execute(String[] args) throws Error
	{
		...
		else if(args[0].equalsIgnoreCase("add"))
		{
			...
			{
				if(tk.wurst_client.features.mods.XRayMod.xrayBlocks.contains(Block
				...
				tk.wurst_client.features.mods.XRayMod.xrayBlocks.add(Block
					.getBlockById(Integer.valueOf(args[2])));
		...
		}else if(args[0].equalsIgnoreCase("remove"))
		{
			if(args[1].equalsIgnoreCase("id") && MiscUtils.isInteger(args[2]))
			{
				for(int i = 0; i < tk.wurst_client.features.mods.XRayMod.xrayBlocks
					.size(); i++)
					if(Integer
						.toString(
							Block
								.getIdFromBlock(tk.wurst_client.features.mods.XRayMod.xrayBlocks
									.get(i))).toLowerCase()
						.equals(args[2].toLowerCase()))
					{
						tk.wurst_client.features.mods.XRayMod.xrayBlocks.remove(i);
						wurst.files.saveXRayBlocks();
